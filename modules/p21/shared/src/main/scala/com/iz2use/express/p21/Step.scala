...
sealed abstract class StepNumber extends Step {
  private[p21] def appendToStringBuilder(builder: StringBuilder): Unit
}
final case class StepFloat(value: Float) extends StepNumber {
  import Step._
  final def foldWith[X](folder: Folder[X]): X = folder.onNumber(this)
  private[p21] def appendToStringBuilder(builder: StringBuilder): Unit = builder.append(value.toString())
...
final case class StepLiteral(value: String) extends Step {
  import Step._
  final def foldWith[X](folder: Folder[X]): X = folder.onLiteral(value)
}
final case class StepObject(tpe: String, fields: Vector[Step]) extends Step {
  ...
  private[p21] def appendToFolder(folder: Printer.PrintingFolder): Unit = {
...
object Step {
  ...
  final def fromLiteral(a: String) = StepLiteral(a)
  ...
  final def fromReference(a: Long): Step = StepReference(a)
