...
trait ObjectEncoder[A] extends RootEncoder[A] { self =>
  final def apply(a: A)(implicit context: EncodingContext): Step = Step.fromStepObject(encodeObject(a))

  def encodeObject(a: A)(implicit context: EncodingContext): StepObject

  final def contramapObject[B](f: B => A): ObjectEncoder[B] = new ObjectEncoder[B] {
    final def encodeObject(a: B)(implicit context: EncodingContext) = self.encodeObject(f(a))
  }

  final def mapStepObject(f: StepObject => StepObject): ObjectEncoder[A] = new ObjectEncoder[A] {
    final def encodeObject(a: A)(implicit context: EncodingContext): StepObject = f(self.encodeObject(a))
  }
}
...
final object ObjectEncoder {
  ...
  final def instance[A](f: A => StepObject): ObjectEncoder[A] = new ObjectEncoder[A] {
    final def encodeObject(a: A)(implicit context: EncodingContext): StepObject = f(a)
  }
