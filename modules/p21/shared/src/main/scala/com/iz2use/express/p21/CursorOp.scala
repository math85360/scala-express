...
final object CursorOp {
  ...
  final case class LeftAt(p: Step => Boolean) extends UnconstrainedOp
  final case class RightAt(p: Step => Boolean) extends UnconstrainedOp
  final case class Find(p: Step => Boolean) extends UnconstrainedOp
  final case class Field(k: String) extends UnconstrainedOp
  final case class DownField(i: String) extends ObjectOp
  final case object DownArray extends ArrayOp
  final case class DownAt(p: Step => Boolean) extends ArrayOp
  ...
  final case class SetLefts(js: Vector[Step]) extends UnconstrainedOp
